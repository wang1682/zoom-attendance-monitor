...
def get_member_display_list(tenant_id: str = None) -> list[dict]:
    """返回 member_display 完整列表，供别名管理页面使用"""
    conn = _get_conn()
    if tenant_id:
        rows = conn.execute(
            "SELECT * FROM member_display WHERE tenant_id = ? ORDER BY display_name",
            (tenant_id,),
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT * FROM member_display ORDER BY display_name"
        ).fetchall()
    result = []
    for r in rows:
        d = dict(r)
        if d.get("aliases"):
            try:
                d["aliases_list"] = json.loads(d["aliases"])
            except:
                d["aliases_list"] = []
        else:
            d["aliases_list"] = []
        d["created_at_display"] = _myt_short(str(d.get("created_at", "")))
        d["updated_at_display"] = _myt_short(str(d.get("updated_at", "")))
        result.append(d)
    return result
...
def update_member_display(
    ...
    try:
        ...
        if display_name is not None:
            ...
            match_key = display_name.strip().lower().replace(" ", "").replace("-", "")
            updates.append("match_key = ?")
            params.append(match_key)
        if note is not None:
            updates.append("note = ?")
            params.append(note.strip())
        if count_enabled is not None:
            updates.append("count_enabled = ?")
            params.append(1 if count_enabled else 0)
        if aliases_json is not None:
            updates.append("aliases = ?")
            params.append(aliases_json.strip())
        if not updates:
            return True
        now = datetime.now(timezone.utc).isoformat()
        ...
        conn.execute(
            f"UPDATE member_display SET {', '.join(updates)} WHERE id = ?",
            params,
        )
        conn.commit()
        return True
