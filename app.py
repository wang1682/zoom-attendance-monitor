    ...
    async def api_v3_member_timeline(display_name: str, request: Request):
        ...
        if source == "history":
            ...
            return {
                ...
                # ── 数据源：participant_sessions（同 get_today_attendance_summary 口径）──
        ps_params = [tenant_id]
        ps_where = ["ps.tenant_id = ?"]
        ps_rows = conn.execute(
            "SELECT ps.user_key, ps.user_name, ps.join_time_utc, ps.leave_time_utc, ps.duration_seconds, ps.meeting_id "
            "FROM participant_sessions ps WHERE " + " AND ".join(ps_where) + " ORDER BY ps.join_time_utc",
            ps_params,
        ).fetchall()
        ...
        for r in ps_rows:
            uk = r["user_key"]
            uname = r["user_name"] or uk
            dn = db.resolve_display_name(uname, tenant_id).get("display_name", uname)
            k = re.sub(r'\\s+', '', dn.lower())
            if k == target_key or target_key in k or k in target_key:
                matched_uks.add(uk)

        if not matched_uks:
            return {"ok": True, "display_name": display_name, "session_count": 0,
                    "disconnect_count": 0, "total_seconds": 0, "total_display": "0m", "segments": [],
                    "data_source": "participant_sessions"}

        uk_list = list(matched_uks)
        ph = ",".join("?" for _ in uk_list)
        member_sessions = conn.execute(
            "SELECT ps.user_key, ps.user_name, ps.join_time_utc, ps.leave_time_utc, ps.duration_seconds, ps.meeting_id "
            "FROM participant_sessions ps WHERE ps.tenant_id = ? AND ps.user_key IN ("
            + ph + ") ORDER BY ps.join_time_utc",
            [tenant_id] + uk_list,
        ).fetchall()
        ...
        for r in member_sessions:
            jt = r["join_time_utc"]
            lt = r["leave_time_utc"]
            jd = datetime.fromisoformat(str(jt).replace("Z", "+00:00"))
            ld = datetime.fromisoformat(str(lt).replace("Z", "+00:00")) if lt else None
            mid = r["meeting_id"]

            if ld:
                dur = max(0, int((ld - jd).total_seconds()))
                lt_disp = _fmt_myt(lt)
                st = "\u2694\ufe0f \u5df2\u79bb\u5f00"
            else:
                dur = max(0, int((now_utc - jd).total_seconds()))
                lt_disp = "\u5728\u7ebf\u4e2d"
                st = "\U0001f7e2 \u5728\u7ebf\u4e2d"

            total_seconds += dur

            if prev_leave_ts and jd > prev_leave_ts:
                gap = (jd - prev_leave_ts).total_seconds()
                if 0 < gap <= merge_window:
                    disconnect_count += 1
            prev_leave_ts = ld if lt else None

            segments.append({
                "seq": len(segments) + 1,
                "enter_time": jt,
                "enter_time_display": _fmt_myt(jt),
                "leave_time": lt,
                "leave_time_display": lt_disp,
                "enter_meeting_id": mid,
                "leave_meeting_id": mid,
                "duration_seconds": dur,
                "duration_display": _fmt_dur(dur),
                "status": st,
            })
